...
namespace sassy {
    ...
    // p.reduce(&g, col, hook);
    //
    // Next, call your GI solver of choice on g, col (graph may have to be adapated)
    ...
    private:
        ...
        void meld_translation_layers() {
            ...
            const int layers = translation_layers.size();
            if(layers == 0) {
                backward_translation_layers.push_back(std::vector<int>());
        ...
        std::pair<int, int>
        select_color_component(sgraph *g, coloring<int> *c1, int component_start_pos, int component_end_pos, int hint) {
            int cell = -1;
        ...
        // assumes internal state of quotient component is up-to-date
        std::pair<int, int>
        select_color_component_large(sgraph *g, coloring<int> *c1, int component_start_pos, int component_end_pos,
                                     int hint) {
            int cell = -1;
            bool only_discrete_prev = true;
            int _i = component_start_pos;
        ...
        // select a color class in a quotient component
        // assumes internal state of quotient component is up-to-date
        std::pair<int, int> select_color_component_large_touched(sgraph *g, coloring<int> *c1, int component_start_pos,
    ...
    private:
        ...
        void compute_quotient_graph_components_update(sgraph *g, coloring<int> *c1, sassy_hook consume) {
        ...
        int
        select_color_component_min_cost(sgraph *g, coloring<int> *c1, int component_start_pos, int component_end_pos,
                                        int max_cell_size) {
            bool only_discrete_prev = true;
            int cell = -1;
            int cell_d = 1;
        ...
        void reset_coloring_to_coloring_touched(sgraph *g, coloring<int> *c_to, coloring<int> *c_from,
    ...
    public:
        void configure(configstruct* config) {
            this->config = config;
        }
        ...
        void reduce(sgraph *g, int *colmap, sassy_hook hook, std::vector<preop> *schedule = &default_schedule) {
            if(config == nullptr) {
            ...
            g->initialize_coloring(&c, colmap);
            refinement<int, int, int> R_stack = refinement<int, int, int>(config);
            R1 = &R_stack;
        ...
        void bliss_save_my_hook(sassy_hook *hook) {
            saved_hook = hook;
        }
        ...
        // nauty/Traces usage specific: (TODO!)
